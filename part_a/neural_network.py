...
class AutoEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = torch.sigmoid(self.g(inputs))  # Apply first layer with sigmoid activation
        out = torch.sigmoid(self.h(out))  # Apply second layer with sigmoid activation
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
        return out
...
def train(model, lr, lamb, train_data, zero_train_data, valid_data, num_epoch):
    ...
    for epoch in range(0, num_epoch):
        ...
        for user_id in range(num_student):
            ...
            # target = inputs.clone()
            inputs = Variable(zero_train_data[user_id]).unsqueeze(0)
            target = Variable(train_data[user_id]).unsqueeze(0)
            ...
            loss += lamb / 2 * model.get_weight_norm()  # Apply regularization
            loss.backward()
...
def evaluate(model, train_data, valid_data, return_pred=False):
    ...
    :param return_pred: whether or not to just return the model predictions
    :return: float or array (if return_pred = True)
    """
...
def main():
    ...
    epochs, train_cost, valid_acc = train(model, lr, lamb, train_matrix,
